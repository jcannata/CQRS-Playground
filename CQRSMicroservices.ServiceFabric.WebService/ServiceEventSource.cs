...
{
  [EventSource(Name = "MyCompany-CQRSMicroservices.ServiceFabric.Application-WebService")]
  internal sealed class ServiceEventSource : EventSource
  {
    ...
    #endregion
    /// <summary>
    ///   Writes an error event message.
    /// </summary>
    /// <param name="message">The message.</param>
    /// <param name="ex">The exception.</param>
    [NonEvent]
    public void ErrorMessage([Localizable(false)] string message, Exception ex)
    {
      ErrorMessage(message, ex.ToString());
    }

    private const int _serviceErrorEventId = 8;

    [Event(_serviceErrorEventId, Level = EventLevel.Error, Message = "{0}")]
    private void ErrorMessage([Localizable(false)] string message, [Localizable(false)] string details)
    {
      WriteEvent(_serviceErrorEventId, message, details);
    }
  }
}
