namespace CQRSMicroservices.ServiceFabric.AggregateRootActor
{
    ...
    internal class AggregateRootActor : Actor, IAggregateRootActor
    {
        ...
        public AggregateRootActor(ActorService service, ActorId id) : base(service, id)
        {
        }
        ...
        private async Task SaveAndDispatchEvents(AggregateRoot root)
        {
            ...
            var eventBusService =
                ServiceProxy.Create<IEventBusService>(
                    new Uri("fabric:/CQRSMicroservices.ServiceFabric.Application/EventBusService"));
